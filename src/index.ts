...
function parseDiffNumStat(output: string): FileChange[] {
  if (!output) return [];
  return output.split('\n').filter(Boolean).map(line => {
    ...
    if (deletions > 0 && additions === 0 && status !== 'renamed') status = 'deleted';
...
function generateSummary(commits: CommitInfo[], changes: FileChange[]): string {
  ...
  }).filter(Boolean) as { type: string; scope: string; desc: string }[];
...
function categorizeFile(file: string): string {
  ...
  if (basename === 'package.json' || basename === 'package-lock.json') return 'Dependencies';
  if (dir.includes('test') || dir.includes('__test') || dir.includes('spec')) return 'Tests';
  if (dir.includes('doc') || ext === '.md') return 'Documentation';
  ...
  if (ext === '.sh' || basename === 'Dockerfile' || basename === 'Makefile') return 'Infra';
  return 'Other';
}

export async function generate(options: GenerateOptions = {}): Promise<PRDescription> {
  ...
  const logOutput = runGit(
    `log --format="%h|||%s|||%an|||%ai" ${base}..${head}`,
    cwd
  );
...
export function formatMarkdown(pr: PRDescription, options: { stats?: boolean } = {}): string {
  const lines: string[] = [];
...
export function formatJson(pr: PRDescription): string {
  return JSON.stringify(pr, null, 2);
}

export function formatText(pr: PRDescription): string {
  const lines: string[] = [];
